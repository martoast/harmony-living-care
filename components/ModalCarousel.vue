<template>
  <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75" @click="closeModal">
    <div class="relative w-full max-w-4xl md:max-w-5xl lg:max-w-6xl p-4 bg-white rounded-lg" @click.stop>
      ...
      <!-- Slider indicators -->
      <div v-if="images.legth > 1" class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3" style="z-index: 100;">
      ...
      <!-- Slider controls -->
      <button v-if="images.legth > 1" @click="prevImage" class="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none">
      ...
      <button v-if="images.legth > 1" @click="nextImage" class="absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none">
        <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 group-hover:bg-white/50 group-focus:ring-4 group-focus:ring-white group-focus:outline-none">
          <svg class="w-4 h-4 text-white rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
